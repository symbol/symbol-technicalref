...
\codenamespace uses a modified version of PoS that attempts to award \emph{users} preferentially relative to \emph{hoarders}.
...
A \emph{Sybil attack}\footnote{See additional discussion in \nemref{sec:finalization}.} on a peer-to-peer network occurs when an attacker
...
Additionally, \nemsetting{network}{importanceActivityPercentage} is 5, so the cumulative transaction score \nemrefparens{sec:consensus:weighting:transaction} accounts for 4\% of importance.
