...
When joining a network, a node first connects to the nominally \textit{static} beacon nodes that are found in a peers configuration file
...
Discovered nodes are called \textit{dynamic} nodes.
...
\codenamespace supports identifying a node by its \textit{resolved host} or its \textit{public key}.
