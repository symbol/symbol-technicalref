...
If any part of a transaction fails to process, the global chain state is reset to the state prior to the transaction application attempt.
...
In order for an aggregate transaction $A$ to pass verification, it must have a cosignature for every embedded transaction signer
...
Like any transaction, an aggregate transaction, must also pass basic transaction signature verification.
