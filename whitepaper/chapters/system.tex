...
\nemchapterfirstletter{H}igh degrees of customization are supported by \codenamespace at both the network level and the individual node level.
Network wide settings specified in \nemsetting{network}{network} must be the same for all nodes in a network.
Node specific settings specified in \nemsetting{node}{node} can vary across nodes in the same network.
...
The transactions a network supports are determined by the set of \textit{transaction plugins} the network requires each node to load.
...
\begin{enumerate}
	...
	\item{Caches -
		...
		Each state-related cache can optionally be included in the calculation of a block's \field{StateHash} (\nemref{sec:blocks:statehash}) when that feature is enabled.
	}
	\item{Handlers - APIs that are always accessible.}
	\item{Diagnostics - APIs and counters that are accessible only when the node is running in diagnostic mode.}
	\item{Validators -
		Stateless and stateful validators that process the notifications produced by block and transaction processing.
	...
	\item{Observers -
		Observers that process the notifications produced by block and transaction processing.
...
The most simple catapult topology is composed of a single server executable.
...
\begin{enumerate}
	\item{Block Versioned Directories -
		...
		The statements (\nemref{sec:blocks:receipts}) generated when processing each block are also stored here for quick access.
		An example of a versioned directory is \filepath{00000}, which contains the first group of blocks.
	}
	\item{\filepath{audit} - Audit files created by the audit consumer (\nemref{sec:disruptor:commonConsumers}) are stored in this directory.}
	\item{\filepath{spool} -
...
Verifiable state (\nemref{sec:blocks:statehash}) requires a cache database and most network configurations are expected to have it enabled.
...
The server raises subscription events in the block chain sync consumer (\nemref{sec:disruptor:blockConsumers}) when it holds an exclusive lock to the blockchain data.
