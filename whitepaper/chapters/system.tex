...
\begin{enumerate}
	\item{Block Versioned Directories -
		...
		Each confirmed block's binary data, transactions and associated data are stored in these directories in files with \filepath{.dat} and \filepath{.stmt} extensions.
		...
		\filepath{hashes.dat} contains a mapping of block heights to block hashes.

		When deterministic finalization is enabled, these directories will also contain versioned \filepath{.proof} files that contain proof information for each finalization epoch.
	}
	...
	\item{\filepath{proof.index.dat} - This is a counter that contains the number of finalization proofs stored on disk.}
	\item{\filepath{voting\_status.dat} - This stores information about the last finalization message sent by a node.}
\end{enumerate}
