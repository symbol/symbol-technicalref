...
Any account that has a network defined \nemsetting{network}{minHarvesterBalance} is eligible to harvest.
...
$$\textit{transaction max fee} >= \nemsetting{node}{minFeeMultiplier} * \textit{transaction size (bytes)}$$
...
\mathit{gh(N)} = \: & H(\textit{gh(N - 1), public key of account})
\end{align*}
